...
DR = 5
LDR = DR + len(dates) - 1
RS = 3  # ref start row in СПРАВОЧНИК
RE = RS + len(ALL) - 1
REF = "СПРАВОЧНИК"
...
def vsum(surf, code, type_name, date_cell=None):
    ...
    criteria = ""
    if date_cell:
        criteria += f",{VN}!$A{VR},{date_cell}"
    criteria += f",{VN}!$C{VR},\"{code}\""
    criteria += f",{VN}!$B{VR},\"{type_name}\""
...
sc(c,B14W,F_TITLE,CTR); sr(ws,1,1,1,16,fill=F_TITLE)
...
# ═══════════════════════════════════════════════════════════
# 3 & 4. ФАКТ sheets — manual entry (option B for СК)
# ═══════════════════════════════════════════════════════════
def build_fact(title, is_finish):
    ws = wb.create_sheet(title)
    ws.sheet_properties.tabColor = "548235" if is_finish else "BF8F00"
    ...
    c=ws.cell(1,1,"ЧИСТОВАЯ ОТДЕЛКА" if is_finish else "ЧЕРНОВАЯ ОТДЕЛКА")
    ...
    for di in range(len(dates)):
        ...
        for cat in ALL:
            ...
            for ci in cols: sc(ws.cell(r,ci),border=BRD,nf='#,##0.00')
        for si in range(3):
...
ws_fin,fc = build_fact("ФАКТ - ЧИСТОВАЯ ОТДЕЛКА",True)
ws_rgh,rc = build_fact("ФАКТ - ЧЕРНОВАЯ ОТДЕЛКА",False)


# ═══════════════════════════════════════════════════════════
# 5. ОТЧЕТ — reads ФАКТ + ВВОД
# ═══════════════════════════════════════════════════════════
...
rr=write_section(ws_rep,rr,"НАДЗЕМНАЯ ЧАСТЬ",ABOVEGROUND)
rr=write_section(ws_rep,rr,"ПОДЗЕМНАЯ ЧАСТЬ",UNDERGROUND)
...
# ═══════════════════════════════════════════════════════════
# 6. SIGNAL — reads ФАКТ + ВВОД (weighted)
# ═══════════════════════════════════════════════════════════
...
for cat in ALL:
    ...
    for di in range(len(dates)):
        ...
        for ci,cd in enumerate(cards):
            ...
            ws_sig.cell(sig_r,bc+1,0); sc(ws_sig.cell(sig_r,bc+1),border=BRD,nf='#,##0.00')
...
# ── Reorder ──
order=["СПРАВОЧНИК","ВВОД","ФАКТ - ЧЕРНОВАЯ ОТДЕЛКА","ФАКТ - ЧИСТОВАЯ ОТДЕЛКА","ОТЧЕТ","SIGNAL"]
for i,name in enumerate(order):
    idx=wb.sheetnames.index(name); wb.move_sheet(name,offset=i-idx)

# ── Save ──
out=r"\\mr.ru\Service\Personal\ignatov_i\Documents\CloudCode\Отделка для SIGNAL v4.xlsx"
wb.save(out)
print(f"OK: {out}")
for n in wb.sheetnames:
    ws=wb[n]; print(f"  {n}: {ws.max_row}r x {ws.max_column}c")
